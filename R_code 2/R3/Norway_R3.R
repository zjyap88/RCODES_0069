...
# 
# ## Retrieve data ##
# norway.mort2 <- hmd.mx("NOR", "zhongjingyap@gmail.com", "Zhongjing@2", "Norway")
# norway.mort <- extract.years(norway.mort2, 1899:2015) 
# norway.mort <- extract.ages(norway.mort, 0:100, combine.upper = TRUE)
# norway.mort$rate$total <- norway.mort$rate$total + 
#   min(norway.mort$rate$total[norway.mort$rate$total > 0])
# sm.norway.mort <- smooth.demogdata(norway.mort)

hz <- 10
M <- matrix(0, nrow = hz, ncol = 6)
M2 <- matrix(0, nrow = hz, ncol = 6)
a <- min(sm.norway.mort[["year"]])
b <- max(sm.norway.mort[["year"]])
cc <- 0
d <- max(sm.norway.mort[["age"]])
m <- 1995
...
for (yr in m:(b - 1)) {
  ...
  test.mort <- extract.years(norway.mort, a:yr)
  sm.test.mort <- extract.years(sm.norway.mort, a:yr)
  h <- min(hz, b - yr)
  ...
  for (age in cc:d) {
    actual <- matrix(log(norway.mort[["rate"]][["total"]])[age + 1, -(1:(yr - a + 1))])
...
# MSE_list_5[["Norway"]] <- results
# MAE_list_5[["Norway"]] <- results2
