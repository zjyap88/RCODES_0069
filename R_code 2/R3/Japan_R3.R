...
# 
# ## Retrieve data ##
# japan.mort2 <- hmd.mx("JPN", "zhongjingyap@gmail.com", "Zhongjing@2", "Japan")
# japan.mort <- extract.years(japan.mort2, 1947:2015) 
# japan.mort <- extract.ages(japan.mort, 0:100, combine.upper = TRUE)
# sm.japan.mort <- smooth.demogdata(japan.mort)

hz <- 10
M <- matrix(0, nrow = hz, ncol = 6)
M2 <- matrix(0, nrow = hz, ncol = 6)
a <- min(sm.japan.mort[["year"]])
b <- max(sm.japan.mort[["year"]])
cc <- 0
d <- max(sm.japan.mort[["age"]])
m <- 1995
...
for (yr in m:(b - 1)) {
  ...
  test.mort <- extract.years(japan.mort, a:yr)
  sm.test.mort <- extract.years(sm.japan.mort, a:yr)
  h <- min(hz, b - yr)
  ...
  for (age in cc:d) {
    actual <- matrix(log(japan.mort[["rate"]][["total"]])[age + 1, -(1:(yr - a + 1))])
...
# MSE_list_5[["Japan"]] <- results
# MAE_list_5[["Japan"]] <- results2
