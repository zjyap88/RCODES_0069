...
randomized_signature <- function(X, k, sigma, Z0 = NULL, A = NULL, b = NULL, seed=1234) {
  set.seed(seed)
  d <- nrow(X)
  N_plus_1 <- ncol(X)
  N <- N_plus_1 - 1
  ...
  if (is.null(Z0)) {
    Z0 <- matrix(rnorm(k), nrow = k, ncol = 1)
  } else {
    Z0 <- matrix(Z0, nrow = k, ncol = 1)
  }
  
  if (is.null(A)) {
    A <- lapply(1:d, function(i) matrix(rnorm(k * k), nrow = k, ncol = k))
  } else {
    stopifnot(length(A) == d)
    for (i in 1:d) {
      stopifnot(nrow(A[[i]]) == k && ncol(A[[i]]) == k)
    }
  }
  if (is.null(b)) {
    b <- lapply(1:d, function(i) matrix(rnorm(k), nrow = k, ncol = 1))
  } else {
    stopifnot(length(b) == d)
    for (i in 1:d) {
      b[[i]] <- matrix(b[[i]], nrow = k, ncol = 1)
    }
  }

  Z <- array(0, dim = c(N_plus_1, k))
  Z[1, ] <- Z0[, 1]
  
  increments <- X[, 2:N_plus_1] - X[, 1:N]  

  for (n in 2:(N_plus_1)) {
    Z_prev <- matrix(Z[n - 1, ], nrow = k, ncol = 1)  
    delta_Z <- matrix(0, nrow = k, ncol = 1)
    for (i in 1:d) {
      Ai_Z_prev <- A[[i]] %*% Z_prev  
      Ai_Z_prev_plus_bi <- Ai_Z_prev + b[[i]]  
      sigma_output <- sigma(Ai_Z_prev_plus_bi)  
      delta_Xi <- increments[i, n - 1]  
      delta_Z <- delta_Z + sigma_output * delta_Xi
    }
    Z[n, ] <- (Z_prev + delta_Z)[, 1]
  }
  
  return(Z)
}

rsigfdm <- function(data, series = names(data$rate)[3], order = 6, ages = data$age, 
                    max.age = max(ages), lambda = 3, mean = TRUE, level = FALSE, transform = TRUE,
                    k=13, activation = function(x) sigmoid(x)){
...
sig <- matrix(0, nrow = nrow(yy), ncol = k)
  for(i in 1:nrow(yy)) {
    zz <- randomized_signature(embed_path(Y[i,]), k, activation)
    sig[i,] <- zz[dim(zz)[1],]
  }
  
  sigphi <- prcomp(sig)[["x"]][,1:order]
  beta <- matrix(0, nrow = n, ncol = order)
...
sigfdm_r1 <- function(data, series = names(data$rate)[3], order = NULL, 
                      var.explained = 0.9, ages = data$age, max.age = max(ages), 
  ...
  # Principal Component Analysis on the signature with centering
  sigphi <- prcomp.sig(sig, center = TRUE, scale. = TRUE)[["x"]][,1:order]
  beta <- matrix(0, nrow = n, ncol = order + 1)  # +1 for intercept
  
  # Linear modeling of the mean-adjusted rates with intercept
  for(i in 1:n) {
    model <- lm(Y[,i] ~ sigphi)  # Include intercept
    beta[i,] <- model$coefficients
  }
  
  beta <- beta[, c(2:ncol(beta), 1)]
  # Replace NA values with 0
  beta[is.na(beta)] <- 0
  
  # Adjust basis and coefficients to include intercept
  sigphi2 <- matrix(NA, nrow = length(x), ncol = order)
  for(i in 1:order) {
    sigphi2[,i] <- approx(xx, sigphi[,i], xout = x)$y
  }
  basis <- cbind(basis, sigphi2)
  coeff <- cbind(coeff, beta[, -dim(beta)[2]])  # Exclude intercept from coefficients
  colnames(basis)[2:(order + 1)] <- paste("Z", 1:order, sep = "")
  colnames(coeff)[2:(order + 1)] <- paste("beta", 1:order, sep = "")
  
  # Adjust for intercept
  intercepts <- beta[,dim(beta)[2]]
  fitted <- basis %*% t(coeff) + matrix(intercepts, nrow = nrow(basis), ncol = n, byrow = TRUE)
  colnames(fitted) <- colnames(mx)
  rownames(fitted) <- rownames(mx)
  ...
  return(list(sig = sig, basis = basis, coef = coeff, intercepts=intercepts, fitted = fits, year = data$year, 
              ages = ages, Y = Y, mean.se = axse, residuals = res, y = data.fts, obs.var = data$obs.var$total,
              pop = data$pop$total, K=order, modelno=1))
}

sigforecast2_r1 <- function(object, h, level = c(80, 95)) {
  nb <- ncol(object$basis)
  order <- nb - 1
  l <- nrow(object$coef)
  ...
  barima_intercept <- auto.arima(ts(object$intercepts))
  pred_intercept <- forecast(barima_intercept, h = h, level = level)
  fc_intercept <- pred_intercept$mean
  
  # Generate forecasts
  fc <- object$basis %*% t(fc_beta) + matrix(fc_intercept, nrow = nrow(object$basis), ncol = h, byrow = TRUE)
  colnames(fc) <- (tail(object$year, 1) + 1):(tail(object$year, 1) + h)
  rownames(fc) <- rownames(object$fitted$y)
  ...
  return(list(forecast = fmean$y, fc_beta = fc_beta, h = h, sigfdm = object, totalvar = totalvar, 
              lower = list(lower80 = lower80, lower95 = lower95), upper = list(upper80 = upper80, upper95 = upper95), ferror = ferror))
  
}


sigfdm_r2 <- function(data, series = names(data$rate)[3], order = NULL,
                      var.explained = 0.9, ages = data$age, max.age = max(ages), 
                      lambda = 3, mean = TRUE, level = FALSE, transform = TRUE,
                      m = 3, Scale){
  if(!is.element(toupper(series), toupper(names(data$rate)))) {
  ...
  # Initialize basis and coefficient matrices
  coeff <- basis <- NULL
  coeff <- cbind(rep(0, n), coeff) #supposed to be 1 not 0
  basis <- cbind(approx(xx, ax, xout = x)$y, basis)
  colnames(coeff)[1] <- colnames(basis)[1] <- "mean"
  ...
  pca <- prcomp.sig(sig, center = FALSE, scale. = FALSE)
  variance <- pca$sdev^2
  prop_variance <- variance / sum(variance)
  cumulative_variance <- cumsum(prop_variance)
  ...
  # Principal Component Analysis on the signature without centering
  sigphi <- prcomp.sig(sig, center = FALSE, scale. = Scale)[["x"]][,1:order]
  beta <- matrix(0, nrow = n, ncol = order)
  
  # Linear modeling of the mean-adjusted rates without intercept
  for(i in 1:n) {
    beta[i,] <- lm(Y[,i] ~ sigphi - 1)$coefficients
  }
  ...
  return(list(sig = sig, basis = basis, coef = coeff, fitted = fits, year = data$year, 
              ages = ages, Y = Y, mean.se = axse, residuals = res, y = data.fts, obs.var = ov,
              pop = data$pop$total, K=order, modelno=2, epsilon = epsil))
}
