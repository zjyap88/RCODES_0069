...
hz <- 10
M <- matrix(0, nrow = hz, ncol = 5)
M2 <- matrix(0, nrow = hz, ncol = 5)
...
for (yr in m:(b - 1)) {
  ...
  rsig.fit <- rsigfdm(sm.test.mort, series = "total", k = 75, activation = function(x) 1/(3*sqrt(13)))
  rsig.fcast <- sigforecast4(rsig.fit, h)
  
  rsig.fit2 <- rsigfdm(sm.test.mort, series = "total", k = 75, activation = function(x) sigmoid::sigmoid(x))
  rsig.fcast2 <- sigforecast4(rsig.fit2, h)
  
  rsig.fit3 <- rsigfdm(sm.test.mort, series = "total", k = 75, activation = function(x) sigmoid::relu(x))
  rsig.fcast3 <- sigforecast4(rsig.fit3, h)
  
  rsig.fit4 <- rsigfdm(sm.test.mort, series = "total", k = 75, activation = function(x) sigmoid::leakyrelu(x))
  rsig.fcast4 <- sigforecast4(rsig.fit4, h)
  
  rsig.fit5 <- rsigfdm(sm.test.mort, series = "total", k = 75, activation = function(x) sigmoid::SoftMax(x))
  rsig.fcast5 <- sigforecast4(rsig.fit5, h)
  
  
  
  mse <- matrix(0, nrow = hz, ncol = 5)
  mape <- matrix(0, nrow = hz, ncol = 5)
  ...
  for (age in cc:d) {
    ...
    data <- cbind(actual[1:length(rsig_fc)], rsig_fc,rsig_fc2,rsig_fc3,rsig_fc4,rsig_fc5)
    data2 <- data
    ...
    while (nrow(se) < hz || is.null(nrow(se))) {
      se <- rbind(se, c(0, 0, 0, 0, 0))
      ape <- rbind(ape, c(0, 0, 0, 0, 0))
    }
...
results <- M
results2 <- M2
colnames(results) <- colnames(results2) <- c("1/(dsqrt(k))", "sigmoid","relu","leakyRelu", "Softmax")
