...
# 
# ## Retrieve data ##
# ireland.mort2 <- hmd.mx("IRL", "zhongjingyap@gmail.com", "Zhongjing@2", "Ireland")
# ireland.mort <- extract.years(ireland.mort2, 1950:2015) 
# ireland.mort <- extract.ages(ireland.mort, 0:100, combine.upper = TRUE)
# sm.ireland.mort <- smooth.demogdata(ireland.mort)

hz <- 10
M <- matrix(0, nrow = hz, ncol = 4)
M2 <- matrix(0, nrow = hz, ncol = 4)
a <- min(sm.ireland.mort[["year"]])
b <- max(sm.ireland.mort[["year"]])
cc <- 0
d <- max(sm.ireland.mort[["age"]])
m <- 1995
...
for (yr in m:(b - 1)) {
  ...
  test.mort <- extract.years(ireland.mort, a:yr)
  sm.test.mort <- extract.years(sm.ireland.mort, a:yr)
  h <- min(hz, b - yr)
  ...
  for (age in cc:d) {
    actual <- matrix(log(ireland.mort[["rate"]][["total"]])[age + 1, -(1:(yr - a + 1))])
...
# MSE_list_5[["Ireland"]] <- results
# MAE_list_5[["Ireland"]] <- results2
