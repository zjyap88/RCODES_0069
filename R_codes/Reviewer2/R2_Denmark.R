...
# 
# ## Retrieve data ##
# denmark.mort2 <- hmd.mx("DNK", "zhongjingyap@gmail.com", "Zhongjing@2", "Denmark")
# denmark.mort <- extract.years(denmark.mort2, 1899:2015) 
# denmark.mort <- extract.ages(denmark.mort, 0:99, combine.upper = TRUE)
# 
# denmark.mort$rate$total <- denmark.mort$rate$total + 
#   min(denmark.mort$rate$total[denmark.mort$rate$total > 0])
# sm.denmark.mort <- smooth.demogdata(denmark.mort)

hz <- 10
M <- matrix(0, nrow = hz, ncol = 4)
M2 <- matrix(0, nrow = hz, ncol = 4)
a <- min(sm.denmark.mort[["year"]])
b <- max(sm.denmark.mort[["year"]])
cc <- 0
d <- max(sm.denmark.mort[["age"]])
m <- 1995
...
for (yr in m:(b - 1)) {
  ...
  test.mort <- extract.years(denmark.mort, a:yr)
  sm.test.mort <- extract.years(sm.denmark.mort, a:yr)
  h <- min(hz, b - yr)
  ...
  # NC.NS.sig.fit <- sigfdm_r2(sm.test.mort, series = "total", m = 2, var.explained = 0.9999, 
  #                            Scale = FALSE)
  # NC.NS.fcast <- sigforecast4(NC.NS.sig.fit, h)
  mse <- matrix(0, nrow = hz, ncol = 4)
  mape <- matrix(0, nrow = hz, ncol = 4)
  
  for (age in cc:d) {
    actual <- matrix(log(denmark.mort[["rate"]][["total"]])[age + 1, -(1:(yr - a + 1))])
