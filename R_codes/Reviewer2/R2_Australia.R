...
# 
# ## Retrieve data ##
# aus.mort2 <- hmd.mx("AUS", "zhongjingyap@gmail.com", "Zhongjing@2", "Australia")
# aus.mort <- extract.years(aus.mort2, 1921:2015) 
# aus.mort <- extract.ages(aus.mort, 0:100,combine.upper = TRUE)
# sm.aus.mort <- smooth.demogdata(aus.mort)

hz <- 10
M <- matrix(0, nrow = hz, ncol = 4)
M2 <- matrix(0, nrow = hz, ncol = 4)
a <- min(sm.aus.mort[["year"]])
b <- max(sm.aus.mort[["year"]])
cc <- 0
d <- max(sm.aus.mort[["age"]])
m <- 1995
...
for (yr in m:(b - 1)) {
  ...
  test.mort <- extract.years(aus.mort, a:yr)
  sm.test.mort <- extract.years(sm.aus.mort, a:yr)
  h <- min(hz, b - yr)
  ...
  for (age in cc:d) {
    actual <- matrix(log(aus.mort[["rate"]][["total"]])[age + 1, -(1:(yr - a + 1))])
    ...
    data <- cbind(actual[1:length(ic.sig_fc)],ic.sig_fc,ncs.sig_fc,select_fc,select_fc2)
    data2 <- data
